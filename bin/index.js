...
var DualOptions_1 = DualOptions$1;

var option = {
	Option: Option_1,
	splitOptionFlags: splitOptionFlags_1,
	DualOptions: DualOptions_1
};
...
var suggestSimilar_2 = suggestSimilar$1;

var suggestSimilar_1 = {
	suggestSimilar: suggestSimilar_2
};
...
const { suggestSimilar } = suggestSimilar_1;
  ...
  _executeSubCommand(subcommand, args) {
    ...
    if (process$1.platform !== 'win32') {
      if (launchWithNode) {
        ...
        proc = child_process.spawn(process$1.argv[0], args, { stdio: 'inherit' });
      } else {
        proc = child_process.spawn(executableFile, args, { stdio: 'inherit' });
      }
    } else {
      args.unshift(executableFile);
      // add executable arguments to spawn
      args = incrementNodeInspectorPort(process$1.execArgv).concat(args);
      proc = child_process.spawn(process$1.execPath, args, { stdio: 'inherit' });
    }
...
var cyanBright = chalk.cyanBright;
...
var treeSort = function treeSort(data, objectPath) {
  ...
  return data.sort(function (a, b) {
    var stateA = fs.statSync(path.join(objectPath, a));
    var stateB = fs.statSync(path.join(objectPath, b));
  ...
  _createClass(EventProcessingCenter, [{
    ...
    value: function useQueryFile(filterFolder) {
      ...
      var buildTree = function buildTree(objectPath) {
        ...
        var docs = treeSort(fs.readdirSync(objectPath), objectPath);
        docs.forEach(function (item) {
          var absolutePath = path.join(objectPath, item);
          var state = fs.statSync(absolutePath);
          /**folder*/
          if (state.isDirectory() && !filterFolder.includes(item)) {
            _this.directory += "".concat(start).concat(directoryEnum.start).concat(item).concat(directoryEnum.line, "\n");
            var level = objectPath.split('\\').length > 1 ? objectPath.split('\\').length - 2 : 0;
            var newStart = directoryEnum.outermost.repeat(level);
            buildTree(path.join("".concat(objectPath, "\\").concat(item)), newStart);
          } else {
            /**file*/
            _this.directory += "".concat(start).concat(directoryEnum.start).concat(item).concat(directoryEnum.line, "\n");
          }
    ...
    value: function run() {
      ...
      spinner.start();
      setTimeout(function () {
        var _this2$useQueryFile = _this2.useQueryFile(_this2.filterFolder),
          buildTree = _this2$useQueryFile.buildTree;
        buildTree(process.cwd());
        fs.writeFile(_this2.fileName, _this2.directory, function (error) {
          if (error) {
            console.log(error(new Error("write ".concat(_this2.fileName, " error"), {
              err: err
            })));
            return;
          }
    ...
    value: function useInquirer(fileName) {
      var _this3 = this;
      var docs = fs.readdirSync(process.cwd());
      this.fileName = fileName;
      if (docs.includes(fileName)) {
        var coverInit = [{
          type: 'confirm',
          name: 'confirm',
          message: warning("".concat(cyanBright(fileName), " already exists in the Target directory ").concat(cyanBright(process.cwd()), " folder, do you want to continue? (y/n)")),
          "default": 'y'
        }];
        inquirer.prompt(coverInit).then(function (_ref) {
...
commander_1.name('directory-doc').version(1212, '-v, --version', 'output the current version');
commander_1.command('init').description('Build the project file directory').option('-i, --ignore [ignore...]', 'You can ignore specific directory name').option('-e, --export <file>', 'You can define the file name for the export', 'directory.md').action(function (options) {
  filterFolder = typeof options.ignore === 'undefined' ? filterFolder : options.ignore;
  console.log(filterFolder);
  var processingCenter = new EventProcessingCenter({
    filterFolder: filterFolder
  });
  processingCenter.useInquirer("".concat(options["export"]));
});
